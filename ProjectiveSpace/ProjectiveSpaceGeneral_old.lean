...
instance : ChartedSpace E ({u : E // u ≠ 0}) := (EstarToE E).singletonChartedSpace 
...
ContDiff.continuous (@Chart2_lift_IsSmooth E _ _ _ _ _ φ v)
...
Chart1 hv (Chart2 hv u) = u := by 
  ...
  have hsimp : (1 / (φ (Projectivization.rep x))) • (Projectivization.rep x) = v + u := by
    have ha := @Projectivization_vs_LinearMap_cor E _ 𝕜 _ _ φ _ _ (Projectivization.rep_nonzero x) hvu 
    erw [hφ1] at ha
